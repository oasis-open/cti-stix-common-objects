...
An Event is an activity that has a harmful effect on the defender/victim.
...
An Impact is the result of an attack to the victim. Impacts can have many catgories: availability of resources, confidentiality of data, integrity of data or resources, monetary, physical damage, damage to others and traceability (auditing). 
...
|[stixtype]#{string_url}[string]#
|The category of impact this object applies to.
...
When populated this impact *MUST* have an *end_time*.
It also *MUST* reference an [stixtype]#<<impact,impact>># of the same as the category specified in the *impact_category* property.
...
If no objects are referenced, the impact should be treated as the overall availability impact for the [stixtype]#{incident_url}[incident]# that references it.
...
A Task is an activity that is performed by or for the victim/defender to respond to the attack.
