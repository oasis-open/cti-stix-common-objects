...
These extensions allow incidents to be tracked across their life cycle where [stixtype]#<<events,events>># are first flagged for investigation resulting in [stixtype]#{incident_url}[incidents]# with [stixtype]#<<task,tasks>># being worked to resolve them.
