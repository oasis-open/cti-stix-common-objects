...
These extensions are allow tracking incidents across their life cycle where Events are first flagged for investigation resulting in Incidents with Tasks being worked to resolve these.
Incidents have impacts that change over time as these Events are determined to have Impacts which are in turn mitigated and potentially resolved by Tasks performed as part of the incident response process.
Both Events and Tasks can exist independently of Incidents and in most workflows will occur prior to an incident being declared.
...
|*activities* (optional)
|[stixtype]#list# of type [stixtype]#activity-entry#
|A list of activities tied to this incident.
Each [stixtype]#activity-entry# *MUST* reference either a Task or Event.
...
This is an open vocabulary and values SHOULD come from the [stixtype]#event-type-ov#.
...
|*subevents* (optional)
|[stixtype]#list# of type [stixtype]#activity-entry#
|A list of sub-activities / events tied to this event.
Each [stixtype]#activity-entry# *MUST* reference a Event.
This list *MUST* not contain cycles.
|===
...
|*subtasks* (optional)
|[stixtype]#list# of type [stixtype]#activity-entry#
|A list of sub-activities / subtasks tied to this task.
Each [stixtype]#activity-entry# *MUST* reference a Task.
This list *MUST* not contain cycles.
|===
...
=== 3.1 Activity Entry Object Type

*Type Name:* [stixtype]#activity-entry#
...
|[stixtype]#identifier# 
|The identity of the event or task that is described by this entry.
Within an Incident this *MUST* reference either a Task or Identity.

When used within a Task this *MUST* only reference other Task objects.
When used within a Event this *MUST* only reference other Event objects.

|*next_steps* (optional)
|[stixtype]#list# of type [stixtype]#activity-sequence-entry#
|A list of activity sequences that describes how the activities that compose the parent object flow into each other.

|===

=== 3.2 Activity Sequence Object Type

Activity sequences entries stored references to a subsequent step for an activity entry.
As these are always stored in an array of steps within an array of activity entries validation rules for *step_ref* *MUST* be performed against the entire array of activity entries.

*Type Name:* [stixtype]#activity-sequence-entry#
...
This reference *MUST* be included as an *activity_ref* within the parent array of [stixtype]#activity-entry# objects.
Within this array there *MUST* not be any cyclical references.

|*condition_type* (required)
|[stixtype]#activity-condition-enum#
...
|[stixtype]#activity-transition-enum#
...
== 6.0 Relationship Summary Table
